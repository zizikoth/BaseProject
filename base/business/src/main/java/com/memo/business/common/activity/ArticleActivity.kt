...
import com.memo.business.databinding.ActivityArticleBinding
...
class ArticleActivity : BaseVmActivity<ArticleViewModel, ActivityArticleBinding>() {
    ...
    companion object {
        fun start(context: Context, url: String, title: String) {
            context.startActivity<ArticleActivity>("url" to url, "title" to title)
        }

        fun start(context: Context, url: String, articleId: Int, title: String) {
            context.startActivity<ArticleActivity>("url" to url, "articleId" to articleId, "title" to title)
        }

        fun start(context: Context, articleId: Int, originId: Int, title: String, url: String) {
            context.startActivity<ArticleActivity>("articleId" to articleId, "originId" to originId, "title" to title, "url" to url)
        }
    }
    ...
    override fun initView() {
        mBinding.run {
            if (articleId != -1) {
                collected = UserManager.hasCollected(articleId)
